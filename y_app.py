...
engine=model_engine, # The 'engine' parameter specifies the name of the OpenAI GPT-3.5 Turbo engine to use.
prompt='\n'.join([f"{p['role']}: {p['content']}" for p in st.session_state['prompts']]), # The 'prompt' parameter is set to the 'prompts' list to provide context for the AI model.
...
generate_response(user_chat_input)
